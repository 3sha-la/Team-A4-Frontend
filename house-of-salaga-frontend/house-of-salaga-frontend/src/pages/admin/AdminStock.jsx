...
import { normalizeProduct, productStatus } from "../../lib/normalizers";
...
const AdminStock = () => {
  ...
  const loadStock = async () => {
    try {
      const data = await apiFetch("/admin/products", { auth: true });
      setStockList(
        (data.products || [])
  ...
  const handleUpdateStock = async (id) => {
    ...
    const item = stockList.find((entry) => entry.id === id);
    if (!item) return;
    ...
    try {
      ...
      setStockList((prev) =>
        prev.map((entry) =>
          entry.id === id
            ? { ...entry, stock: newStock, status: productStatus(newStock) }
            : entry,
        ),
      );
  ...
  const outOfStockCount = stockList.filter((item) => item.stock <= 0).length;
  const lowStockCount = stockList.filter(
    (item) => item.stock > 0 && item.stock <= 5,
  ).length;
  const inStockCount = stockList.filter((item) => item.stock > 5).length;
  const totalVariants = stockList.length || 1;
  const inStockPct = Math.round((inStockCount / totalVariants) * 100);
  const lowStockPct = Math.round((lowStockCount / totalVariants) * 100);
  const outStockPct = Math.round((outOfStockCount / totalVariants) * 100);

  return (
    <div className="max-w-[92%] mx-auto space-y-6 font-sans text-[#1A1A1A]">
      <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-[#212121] text-white p-6 rounded-2xl flex flex-col justify-between">
          ...
          <p className="text-[11px] text-[#E5A800] mt-4 font-medium">
            Across {stockList.length} product variants
          </p>
        </div>
            ...
            <div className="space-y-3 text-xs">
              <div>
                <div className="flex justify-between text-gray-600 mb-1">
                  <span>In Stock ({inStockPct}%)</span>
                  <b className="text-black">{inStockCount} items</b>
                </div>
              ...
              <div>
                <div className="flex justify-between text-gray-600 mb-1">
                  <span>Low Stock ({lowStockPct}%)</span>
                  <b className="text-black">{lowStockCount} items</b>
                </div>
                <div className="w-full bg-stone-100 h-1.5 rounded-full overflow-hidden">
                  <div
                    className="bg-[#E5A800] h-full"
              ...
              <div>
                <div className="flex justify-between text-gray-600 mb-1">
                  <span>Out of Stock ({outStockPct}%)</span>
                  <b className="text-black">{outOfStockCount} items</b>
                </div>
                <div className="w-full bg-stone-100 h-1.5 rounded-full overflow-hidden">
                  <div
