...
const AdminCategories = () => {
  ...
  const addActivityLog = (name, action, color = "text-gray-500") => {
    const newLog = { id: Date.now(), name, action, time: "Just now", color };
  ...
  const handleSaveCategory = async (e) => {
    ...
    try {
      if (editingId) {
        const oldCategory = categories.find((cat) => cat.id === editingId);
        if (!oldCategory) return;

        if (
          oldCategory.name.toLowerCase() !== categoryName.trim().toLowerCase()
        ) {
          await apiFetch("/admin/categories", {
            method: "POST",
            auth: true,
            body: { name: categoryName.trim(), description },
          });
          const affected = products.filter(
            (product) =>
              product.category.toLowerCase() === oldCategory.name.toLowerCase(),
          );
          await Promise.all(
            affected.map((product) =>
              apiFetch(`/admin/products/${product.id}`, {
                method: "PUT",
                auth: true,
                body: { category: categoryName.trim() },
              }),
            ),
          );
  ...
  const filteredCategories = categories
    ...
    .sort((a, b) => {
      ...
      if (sortBy === "count")
        return (
          getProductCount(b.tag, b.count) - getProductCount(a.tag, a.count)
        );
      return 0;
    });
  ...
  return (
    <div className="max-w-[92%] mx-auto space-y-6 font-sans text-[#1A1A1A] relative">
